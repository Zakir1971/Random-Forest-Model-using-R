...
load("C:\\Users\\neelb\\Downloads\\SLEEP QUALITY PROJECT\\e4 data\\Final sleep quality models.RData")


##feeling refreshed model
load("C:\\Users\\neelb\\Downloads\\SLEEP QUALITY PROJECT\\e4 data\\Final feeling refreshed models.RData")

########### load test data
# The address of E4 data
root="C:\\Users\\neelb\\Downloads\\SLEEP QUALITY PROJECT\\e4 data\\"; #try with and without \e4 data
numberPerson = 1
numberWeek = 1
numberDay = 1
...
for (p_i in 1:numberPerson){
  for (w_i in 1:numberWeek){
    for (d_i in 1:numberDay){
      ...
      setwd("/Users/neelb/Downloads/SLEEP QUALITY PROJECT/e4 data/Data/Participant 1/Week 1/1")
      if(!Flage)
      {
        next
      }
      print(direction)
      ACC <- read.csv("ACC.csv",header = FALSE,as.is = TRUE)
      data [[p_i]][[w_i]][[d_i]]$ACC <- ACC
      
      BVP <- read.csv("BVP.csv",header = FALSE,as.is = TRUE)
      data [[p_i]][[w_i]][[d_i]]$BVP <- BVP
      
      EDA <- read.csv("EDA.csv",header = FALSE,as.is = TRUE)
      data [[p_i]][[w_i]][[d_i]]$EDA <- EDA
      
      HR <- read.csv("HR.csv",header = FALSE,as.is = TRUE)
      data [[p_i]][[w_i]][[d_i]]$HR <- HR
      
      IBI <- read.csv("IBI.csv",header = FALSE,as.is = TRUE)
      data [[p_i]][[w_i]][[d_i]]$IBI <- IBI
      
      Temp <- read.csv("Temp.csv",header = FALSE,as.is = TRUE)
...
#t1 <- data.frame(`Particiapnt #`,Week,`Survey Number`,stringsAsFactors = FALSE)
...
library(R.matlab)
#options(matlab = "C:\\Applications\\MATLAB_R2020a.app\\bin\\matlab") #ADDED LINE
Matlab$startServer()
matlab <- Matlab()
isOpen <- open(matlab)
print(matlab)
...
for (i in 1:dim(test)[1]) {
  print(test[i,c(1:3)])
  nick= test[i,4:dim(test)[2]]
  class(nick)
  
  try(print(predict(fit.rf, nick)))
}
...
stripchart(test[which(test$`Particiapnt #`==p_i & test$Week==w_i & test$`Survey Number`==d_i),selectedFeatures],
           vertical = TRUE, method = "jitter", add=TRUE, pch=20, col='red',cex =3)
...
stripchart(test[which(test$`Particiapnt #`==p_i & test$Week==w_i & test$`Surve Number`==d_i),selectedFeatures],
           vertical = TRUE, method = "jitter", add=TRUE, pch=20, col='red',cex =3)
...
plot.ts(test[,c(12:14)],xlab='nights',main='The trends of signal-based features in different nights', ylab= "hi")
